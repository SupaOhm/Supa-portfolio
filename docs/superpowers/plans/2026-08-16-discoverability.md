...
Run: `npm test -- scripts/discoverability.test.ts`
Expected: **FAIL**, all 12 new tests (`carries exactly one ld+json block` reports `[]` has length 0; the rest fail on an empty graph). The 11 Task 2 tests still pass.

`publishes no email address` fails here too, but by a different mechanism worth knowing: `nodeOfType('Person')` is `undefined` with no block present, and `expect(undefined).not.toHaveProperty(...)` does not pass vacuously — Vitest 4.1 raises `TypeError: Cannot convert undefined or null to object`. Probed directly on this project's Vitest. So the red is a thrown error, not an assertion failure, and that is expected at this step.
